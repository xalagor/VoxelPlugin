...
struct FVoxelMeshConfig
{
	...
	ERendererStencilMask CustomDepthStencilWriteMask = static_cast<ERendererStencilMask>(0);

	/** Optionally write this 0-255 value to the stencil buffer in CustomDepth pass (Requires project setting or r.CustomDepth == 3) */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Voxel",
		meta=(UIMin = "0", UIMax = "255", EditCondition="bRenderCustomDepth", DisplayName = "CustomDepth Stencil Value"
		))
	int32 CustomDepthStencilValue = 0;

	template <typename T>
	auto& ApplyTo(T& Mesh) const
	{
...
inline uint32 GetTypeHash(const FVoxelMeshConfig& Config)
{
	return
		...
		static_cast<uint8>(Config.CustomDepthStencilWriteMask) * 56453 +
